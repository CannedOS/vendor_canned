...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Canned model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function cannedremote()
{
    ...
    git remote rm canned 2> /dev/null
    ...
    local CANNED_USER=$(git config --get review.review.canned.org.username)
    if [ -z "$CANNED_USER" ]
    then
        git remote add canned ssh://review.canned.org:29418/$PFX$PROJECT
    else
        git remote add canned ssh://$CANNED_USER@review.canned.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'canned' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/CannedOS/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        cannedremote
        git push canned HEAD:refs/heads/'$1'
    fi
    '
}

function cannedgerrit() {
    ...
    local user=`git config --get review.review.canned.org.username`
...
function cannedrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "CannedOS Gerrit Rebase Usage: "
        echo "      cannedrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.canned.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                canned|otapackage|bacon|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
