...
// Copyright (C) 2018 The CannedOS Project
//
...
func init() {
	android.RegisterModuleType("canned_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cannedExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cannedExpandVariables(ctx, dep_file)
	...
	cmd := cannedExpandVariables(ctx, String(g.properties.Cmd))
