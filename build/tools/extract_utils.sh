...
#               2017-2020 The CannedOS Project
#
...
function setup_vendor() {
    ...
    export CANNED_ROOT="$3"
    if [ ! -d "$CANNED_ROOT" ]; then
        echo "\$CANNED_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$CANNED_ROOT/$OUTDIR" ]; then
        mkdir -p "$CANNED_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$CANNED_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$CANNED_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$CANNED_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$CANNED_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
    ...
    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$CANNED_ROOT"/prebuilts/tools-canned/${HOST}-x86/bin/patchelf
    fi
}
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR The CannedOS Project\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR The CannedOS Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The CannedOS Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The CannedOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The CannedOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The CannedOS Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The CannedOS Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The CannedOS Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$CANNED_ROOT"/prebuilts/tools-canned/common/smali/baksmali.jar
        export SMALIJAR="$CANNED_ROOT"/prebuilts/tools-canned/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$CANNED_ROOT"/prebuilts/tools-canned/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$CANNED_ROOT"/prebuilts/tools-canned/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$CANNED_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$CANNED_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$CANNED_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$CANNED_ROOT"/vendor/canned/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$CANNED_ROOT"/"$OUTDIR"/radio
